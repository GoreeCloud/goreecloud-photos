	...
	if err := p.pool.QueryRow(probeContext, `
		SELECT
			...
			EXISTS (
				...
				WHERE table_schema = 'public' AND table_name = 'upload_sessions' AND column_name = 'original_filename'
			) AND
			EXISTS (
				SELECT 1 FROM information_schema.columns
				WHERE table_schema = 'public' AND table_name = 'upload_sessions' AND column_name = 'media_type'
			) AND
			EXISTS (
				SELECT 1 FROM information_schema.columns
				WHERE table_schema = 'public' AND table_name = 'upload_sessions' AND column_name = 'device_id'
			)
